...
description: All nuts and bolts needed to go from quaternions to a quaternion rotation operator
...
> **Citation Note:** I have borrowed this three-part series' content from Jack B. Kuipers' seminal 1999 book <d-cite key="kuipers1999quaternions"></d-cite> on quaternions.  

> As an admirer of the book's writing style, I submit that you are probably better off reading the Chapter 5 of the book. My motivation to write this regardless is two fold - to provide robotics practitioners a one-stop-shop entry point to quaternions as well as challenge/improve my understanding of them. I claim no expertise in this topic but found it so fascinating that I decided to write about it. 

> **Prerequisites:** This is [2/3] article in my three part series on quaternions. For this article, I assume readers' familiarty with basic quaternion algebra <d-cite key="quat-basics"></d-cite> and rotation matrices and their basic properties<d-cite key="rotmat"></d-cite><d-cite key="rotmat2d"></d-cite><d-cite key="rotmat3d"></d-cite>. 
***
...
By the end of this article, we will see that this checklist is indeed fulfilled with quaternions albeit with some upgrades to quaternions we already know.
...
* Quaternion multiplication between two pure quaternions 
* Quaternion multiplication between a scalar and a pure quaternion 

provide a _one-to-one correspondence_ between $$\mathbb{R}^3$$ and $$Q_0$$. This is great and goes in the direction of items 1 and 2 in our checklist.

{% include figure.html path="assets/img/blog/pure_quat.png" width="250" height="125" class="rounded"  caption="Source: Jack B Kuipers. Quaternions and Rotation Sequences (1999)" %}

However, multiplication (See Eq. $$(1)$$ from <d-cite key="quat-basics"></d-cite> for clarity) between a general and a pure quaternion doesn't _always_ give us the same desirable one-to-one mapping but it exists and is as follows: 
...
For quaternions to operate on vectors in $$\mathbb{R}^3$$, it would have helped a lot in favour of item 2 in our list if their multiplication lead to a one-to-one correspondence between $$\mathbb{R}^3$$ and $$Q_0$$. However, we see in Eq. (DOUBLE PRODUCT) that the _double_ product still has a scalar part. The _double_ products $$\mathbf{q} \star \mathbf{v}$$ and $$\mathbf{v} \star \mathbf{q}$$ do not have a place in $$R_{\mathbf{q}}$$. This suggests that the quaternion operator $$R_{\mathbf{q}}$$ must have a _triple_ or evern higher order quaternion multiplication in it. 

As one should, lets see if a _triple_ product gives us what we want and what we want is to be able to stay in $$\mathbb{R}^3$$ after the mulltiplication. Consider two general quaternions $$\mathbf{p}=p_0 + \vec{p}$$ and $$\mathbf{q} = q_0 + \vec{q}$$ and a pure quaternion $$\mathbf{v} = 0 + \vec{v}$$. There are six possible products involving these three quaternions:

$$
\mathbf{p} \star \mathbf{q} \star \mathbf{v} \quad \mathbf{q} \star \mathbf{v} \star \mathbf{p} \quad  \mathbf{v} \star \mathbf{p} \star \mathbf{q} \quad \mathbf{q} \star \mathbf{p} \star \mathbf{v} \quad \mathbf{p} \star \mathbf{v} \star \mathbf{q} \quad \mathbf{v} \star \mathbf{q} \star \mathbf{p}
$$
...
Despite the uninteresting exercise in it, lets expand one the _triple_ products, say $$\mathbf{p} \star \mathbf{v} \star \mathbf{q}$$.
...
&= - q_0 (\vec{p}\cdot\vec{v}) - p_0(\vec{v} \cdot \vec{q}) - (\vec{p} \times \vec{v}) \cdot \vec{q} - (\vec{p}\cdot\vec{v})\cdot\vec{q} \\
&\quad + q_0(p_0\vec{v} + \vec{p} \times \vec{v}) + (p_0\vec{v} + \vec{p} \times \vec{v}) \times \vec{q}
\end{aligned}
$$
...
It is easy to verify that the above expression goes to $$0$$ when $$\vec{p}$$ and $$\vec{v}$$ are parallel to each other or if $$k=-1$$ and $$p_0 = q_0$$ which would simply mean that

$$
\mathbf{p} = p_0 + \vec{p} = q_0 -\vec{q} \Rightarrow \mathbf{p} = \mathbf{q}^{\ast}
$$

From this discussion, we obtain two _triple_ quaternion products that always output a pure quaternion whenever they _sandwich_ (hence the name) a pure quaternion, although it is not yet clear how they differ from each other (Eq. SANDWICH here)
...
<!-- In fact, even for higher order multiplication with unit quaternions, we see that the norm of the factors is preserved. Say, we have $$n$$ unit quaternions $$\{\mathbf{q}_1, \mathbf{q}_2, ..., \mathbf{q}_n\}$$ and a general quaternion $$\mathbf{p}$$. The squared norm of the product

$$
\begin{aligned}
\Vert \mathbf{q}_1 \star ... \star \mathbf{p} \star ... \star \mathbf{q}_n \Vert^2 &= \Vert \mathbf{p} \star \mathbf{q}_1 \star ... \star \mathbf{q}_n \Vert^2 = \Vert \mathbf{q}_1 \star ... \star \mathbf{q}_n \star \mathbf{p} \Vert^2 = \Vert \mathbf{p} \Vert^2
\end{aligned}
$$ -->
...
Complex numbers<d-footnote>I bring complex numbers into the discussion for familiarity and easy visualizations. If this is obvious to you, please skip to the next subsection.</d-footnote> have an intuitive alternate form of representation to them. A complex number $$z = a + ib$$ can we interpreted as a 2D vector on a complex plane where the horizontal axis is the real axis and the vertical axis is the imaginary axis, as shown in the figure below. A natural consequence of this is the polar or trignometric form where $$z = a + ib$$ can be represented with its polar pair $$(r, \theta)$$ such that $$r = \sqrt{a^2 + b^2}$$ (norm) and $$\theta = \arctan (\frac{b}{a})$$ (angle between complex vector and the real axes), from this it follows that $$\cos \theta = \frac{a}{r}$$ and $$\sin \theta = \frac{b}{r}$$. 

{% include figure.html path="assets/img/blog/complex_plane.png" width="125" height="250" class="rounded"%}
...
&= r_1r_2(\cos \theta_1 \cos \theta_2 + i \cos \theta_1 \sin \theta_2 + i \sin \theta_1 \cos \theta_2 - \sin \theta_1 \sin \theta_2) \\
&= r_1r_2(\cos \theta_1 \cos \theta_2 - \sin \theta_1 \sin \theta_2 + i (\cos \theta_1 \sin \theta_2 + \sin \theta_1 \cos \theta_2)) \\
&= r_1r_2(\cos (\theta_1 + \theta_2) + i \sin (\theta_1 + \theta_2)) \quad \text{(from def. of} \cos (\theta_1 + \theta_2) \text{ and } \sin (\theta_1 + \theta_2))
\end{aligned}
$$

The geometric interpretation of the complex product is clear now. When two complex numbers are multiplied, you notice that the output complex number has an associated angle that is exactly the sum of angles associated with the multiplicands and has magnitude equal to the product of magnitudes of the multiplicands. 

{% include figure.html path="assets/img/blog/complex_product.png" width="125" height="250" class="rounded" caption="This is true if a3+ib3 = (a1+ib1)(a2+ib2)"%}

We can also verify that multiplying a complex number with another complex number's conjugate essentially subtracts or _cancels_ the former's angle by later's angle. All of this is quite suggestive of the fact that complex numbers can be used to represent 2D rotations (also scaling for non-unit complex numbers), a fact that is perhaps obvious or well known to many. However, does this extend to quaternions? Yes. No. Maybe. We hope to find out soon!

### Quaternions

Quaternions, particularly unit quaternions, can also be written in polar form albeit the interpretation of the angle $$\theta$$ may not be as straightforward as it is for complex numbers. We know that a unit quaternion $$\mathbf{q} = q_0 + \vec{q}$$ has norm 1 i.e., $$q_0^2 + \vec{q}^2 = 1$$. And that for any angle $$\theta$$, we know that $$\cos^2 \theta + \sin^2 \theta = 1$$ holds. So there must be an angle $$\theta$$ such that 
...
The angle $$\theta$$ can be defined uniquely as long as it stays within $$-\pi < \theta \leq \pi$$. But this is it, we now have an angle (althought unclear what it represents) associated with the quaternion $$\mathbf{q}$$. We can take this form further by defining a unit vector $$\vec{u}$$, which represents the direction of $$\vec{q}$$:

$$
\vec{u} = \frac{\vec{q}}{\Vert \vec{q} \Vert} = \frac{\vec{q}}{\sin \theta}
$$

Then we may be able to write all unit quanternions in terms of the angle $$\theta$$ and the unit vector $$\vec{u}$$ as 
...
Looking back at Eq. (SANDWICH), if we replace $$\theta$$ by $$-\theta$$ of $$\mathbf{q}$$ in one operator, we get the other operator. Hence, by the appropriate choice of the angle $$\theta$$ these operators may in fact represent the same geometric transformation. 

### A Special Property of Quaternion Product

I hope you have figured out where I am going with this by now. Consider two unit quaternions:
...
&= \cos \theta_1 \cos \theta_2 - (\vec{u} \sin \theta_1) \cdot (\vec{u} \sin \theta_2) + \cos \theta_1 (\vec{u} \sin \theta_2) + \cos \theta_2 (\vec{u} \sin \theta_1) + \vec{u} \sin \theta_1 \times \vec{u} \sin \theta_2 \\
&= \cos \theta_1 \cos \theta_2 - \sin \theta_1 \sin \theta_2 + \vec{u} (\cos \theta_1 \sin \theta_2 + \sin \theta_1 \cos \theta_2)  + 0\\
&= \cos (\theta_1 + \theta_2) + \vec{u} \sin (\theta_1 + \theta_2)
\end{aligned}
$$

Once again, very similar to complex numbers, this is an interesting result and has an important geometric implication. It says, if we multiply two unit quaternions $$\mathbf{q}_1$$ and $$\mathbf{q}_2$$, each having the same unit vector in them $$\vec{u}$$, then the product is also a unit quaternion having this same unit vector $$\vec{u}$$. And, associated with it is an angle that is exactly the sum of angles associated with $$\mathbf{q}_1$$ and $$\mathbf{q}_2$$. Now if, in fact, quaternion rotation operator represents rotation, this is exactly what we would expect and desire.
