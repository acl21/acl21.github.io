...
{% include figure.html path="assets/img/blog/quat-rot-viz.png" class="img-fluid rounded img-width-20" zoomable=true %}
...
{% include figure.html path="assets/img/blog/quat-rot-viz.gif" class="rounded img-width-30"  caption="The visualisation shows rotation from the above example but with varying associated angle." zoomable=true %}
...
While the conjectural proof above may be intuitive and perhaps sufficient for some readers, I refer the more curious to Chapter 15.15 of <d-cite  key="kuipers1999quaternions"></d-cite> for the full formal proof. I also encourage readers to check the subsequent Chapter 15.16 for discussion, proof and examples on how these operators elegantly handle rotation sequences. The quaternion operator sequence theorems are as follows:
...
> **Theorem 4:** Suppose that $$\mathbf{p}$$ and $$\mathbf{q}$$ are unit quaternions which define the quaternion rotation operators $$R_{\mathbf{p}}(\vec{u}) = \mathbf{p}^{\ast} \star \vec{u} \star \mathbf{p}$$ and $$R_{\mathbf{q}}(\vec{v}) = \mathbf{q}^{\ast} \star \vec{v} \star \mathbf{q}$$. Then the quaternion product $$\mathbf{p} \star \mathbf{q}$$ defines a quaternion rotation operator $$R_{\mathbf{p} \star \mathbf{q}}$$ which represents a sequence of operators, $$R_{\mathbf{p}}$$ followed by $$R_{\mathbf{q}}$$. The axis and the angle of rotation are those represented by the quaternion product $$\mathbf{p} \star \mathbf{q}$$.

Note that in Theorem 3 and Theorem 4, the sequence of operators is the same i.e., $$R_{\mathbf{p}}$$ followed by $$R_{\mathbf{q}}$$. I leave you with something to ponder on your next coffee break. 

## Conclusion

This concludes my attempt to get robotics practitioners introduced to quaternions and their rotation operators. Perhaps now is a good time to watch Grant Sanderson a.k.a Mathematics YouTuber 3Blue1Brown's videos on quaternions<d-cite key="3b1b"></d-cite>. 
