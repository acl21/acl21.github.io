...
Without a formal proof, I claim that these operators indeed rotate $$\vec{v}$$ in $$\mathbb{R}^3$$. Through some carefully designed tests, I hope to convince you on the same by answering the following key questions about $$R_{\mathbf{q}}$$:
- **Q1. What is the direction of rotation?**
- **Q2. What is the axis of rotation?**
- **Q3. What is the angle of rotation?**
- **Q4. What is the difference between $$R_{\mathbf{q}}$$ and $$R_{\mathbf{q}^{\ast}}$$?**
...
Assume that the associated angle $$\theta$$ is a very small positive angle. This assumption is helpful when comparing the input vector with the operator's output vector. We want to verify if $$R_{\mathbf{q}}(\vec{v})$$ merely "tweaks" the input vector when the associated angle is a very small positive value. Conveniently, for very small angles, we can round things up nicely i.e., $$\cos \theta \approx 1$$ and $$\sin \theta \approx \theta$$ so the quaternion $$\mathbf{q}$$ may be re-written as 

$$
\mathbf{q} \approx 1 + k\theta
$$

In the spirit of simplicity, suppose that you want to rotate another basis vector $$\vec{v} = 1i + 0j + 0k = i$$ (or $$\left[\begin{matrix} 1 \\ 0 \\ 0\end{matrix}\right]$$). Lets see what the operator $$R_{\mathbf{q}}(\vec{v})$$ outputs for this carefully chosen $$\mathbf{q}$$ and $$\vec{v}$$:
...
&= (1+k\theta)(i)(1-k\theta) \\
&= (1+k\theta)(i+j\theta) \\
\vec{w} &= i + 2\theta j \quad (\text{or }\left[\begin{matrix} 1 \\ 2\theta \\ 0\end{matrix}\right]) \quad \hspace{5cm} \text{(1)}
\end{aligned}
$$
...
{% include figure.html path="assets/img/blog/quat-rot-viz.gif" width="500" height="250" class="rounded"  caption="The visualization shows rotation from the above example but with varying associated angle." %}
...
When I first saw the incremental test, I was left uneasy and skeptical of the angle of rotation due to the approximations made. But we can be sure of the claims made on angle of rotation with a simple angle test. Let us choose a unit quaternion with the associated angle $$\theta=\frac{\pi}{6}$$ while keeping the axis of rotation $$\vec{u}$$ as the same basis vector $$k$$. Then we have the following quaternion
